...
export class UsersService {
  ...
  async createUser(user: CreateUser) {
    const exists = await this.databaseService.isExists(this.userModel, { email: user.email });
    if (exists) {
  ...
  async getUser(email: string) {
    return await this.databaseService.getSingleItem(this.userModel, { email: email });
  }

  async updateUser(user: UpdateUser, email: string) {
    return await this.databaseService.updateItem(this.userModel, { email: email }, user);
  }

  async deleteUser(email: string) {
    return await this.databaseService.deleteItem(this.userModel, { email: email });
  }
}
